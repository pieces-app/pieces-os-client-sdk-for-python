...
class TLPCodeFragmentStatistics(BaseModel):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> TLPCodeFragmentStatistics:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic.v1 by calling `to_dict()` of descriptive
        if self.descriptive:
            _dict['descriptive'] = self.descriptive.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> TLPCodeFragmentStatistics:
        """Create an instance of TLPCodeFragmentStatistics from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return TLPCodeFragmentStatistics.parse_obj(obj)

        _obj = TLPCodeFragmentStatistics.parse_obj({
            "var_schema": EmbeddedModelSchema.from_dict(obj.get("schema")) if obj.get("schema") is not None else None,
            "descriptive": TLPCodeFragmentDescriptiveStatistics.from_dict(obj.get("descriptive")) if obj.get("descriptive") is not None else None
        })
        return _obj
