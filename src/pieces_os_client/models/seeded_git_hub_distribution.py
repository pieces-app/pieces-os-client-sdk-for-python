...
class SeededGitHubDistribution(BaseModel):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> SeededGitHubDistribution:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic.v1 by calling `to_dict()` of gist
        if self.gist:
            _dict['gist'] = self.gist.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> SeededGitHubDistribution:
        """Create an instance of SeededGitHubDistribution from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return SeededGitHubDistribution.parse_obj(obj)

        _obj = SeededGitHubDistribution.parse_obj({
            "var_schema": EmbeddedModelSchema.from_dict(obj.get("schema")) if obj.get("schema") is not None else None,
            "gist": SeededGitHubGistDistribution.from_dict(obj.get("gist")) if obj.get("gist") is not None else None
        })
        return _obj
