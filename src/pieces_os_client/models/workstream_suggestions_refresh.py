...
class WorkstreamSuggestionsRefresh(BaseModel):
    """
    ...
    @classmethod
    def from_json(cls, json_str: str) -> WorkstreamSuggestionsRefresh:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic.v1 by calling `to_dict()` of suggestions
        if self.suggestions:
            _dict['suggestions'] = self.suggestions.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> WorkstreamSuggestionsRefresh:
        """Create an instance of WorkstreamSuggestionsRefresh from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return WorkstreamSuggestionsRefresh.parse_obj(obj)

        _obj = WorkstreamSuggestionsRefresh.parse_obj({
            "var_schema": EmbeddedModelSchema.from_dict(obj.get("schema")) if obj.get("schema") is not None else None,
            "suggestions": WorkstreamSuggestions.from_dict(obj.get("suggestions")) if obj.get("suggestions") is not None else None
        })
        return _obj
