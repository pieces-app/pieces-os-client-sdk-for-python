...
class TestBasicAsset:
    ...
    def pieces_client(self):
        return PiecesClient()

    @pytest.fixture(scope="function", autouse=True)
    def setup_asset_snapshot(self, pieces_client):
    ...
    @pytest.fixture
    def mock_asset_snapshot(self, mock_asset):
    ...
    def test_basic_asset_initialization(self, mock_asset_snapshot, mock_asset):
        ...
        asset = self.BasicAssetTest(mock_asset.id)
        assert asset.asset == mock_asset

    def test_raw_content_property(self, mock_asset_snapshot, mock_asset):
        asset = self.BasicAssetTest(mock_asset.id)
        assert asset.raw_content == "Test content"

    def test_is_image(self, mock_asset_snapshot, mock_asset):
        asset = self.BasicAssetTest(mock_asset.id)
    ...
    def test_classification_property(self, mock_asset_snapshot, mock_asset):
        asset = self.BasicAssetTest(mock_asset.id)
        assert asset.classification == ClassificationSpecificEnum.PY

    def test_edit_content(self, mock_asset_snapshot, mock_asset, pieces_client):
        asset = self.BasicAssetTest(mock_asset.id)
    ...
    def test_edit_name(self, mock_asset_snapshot, mock_asset, pieces_client):
        asset = self.BasicAssetTest(mock_asset.id)
    ...
    def test_name_property(self, mock_asset_snapshot, mock_asset):
        asset = self.BasicAssetTest(mock_asset.id)
    ...
    def test_description_property(self, mock_asset_snapshot, mock_asset):
        asset = self.BasicAssetTest(mock_asset.id)
    ...
    def test_annotations_property(self, mock_asset_snapshot, mock_asset):
        asset = self.BasicAssetTest(mock_asset.id)
    ...
    def test_delete(self, mock_asset_snapshot, mock_asset, pieces_client):
        asset = self.BasicAssetTest(mock_asset.id)
    ...
    def test_create(self, pieces_client):
        with patch.object(pieces_client, 'assets_api') as mock_assets_api:
            mock_assets_api.assets_create_new_asset.return_value = MagicMock(id="new_asset_id")

            new_asset_id = self.BasicAssetTest.create("New asset content")
